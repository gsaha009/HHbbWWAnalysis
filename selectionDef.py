...
def makeSingleLeptonSelection(self,baseSel,plot_yield=False,use_dd=True): 
    ...
    #---- Mll cuts ----#
    ElSelObj.selName += "PreMllCut"
    MuSelObj.selName += "PreMllCut"
    ElSelObj.yieldTitle += " + $M_{ll}>12$"
    MuSelObj.yieldTitle += " + $M_{ll}>12$"
    ...
    ElSelObj.refine(cut = mllCut)
    MuSelObj.refine(cut = mllCut)


    if not self.args.NoZVeto: 
        ElSelObj.selName += "OutZ"
        MuSelObj.selName += "OutZ"
        ElSelObj.yieldTitle += " + Z Veto $|M_{ll}^{Preselected OS SF}-M_Z|>10$"
        MuSelObj.yieldTitle += " + Z Veto $|M_{ll}^{Preselected OS SF}-M_Z|>10$"
        outZCut = [lambda_outZ(self.OSElElDileptonPreSel),lambda_outZ(self.OSMuMuDileptonPreSel)]
        ElSelObj.refine(cut = outZCut)
        MuSelObj.refine(cut = outZCut)
    ...
    if not self.args.NoTauVeto: # nTau (isolated from fakeable leptons) = 0
        ...
        ElSelObj.selName += "noTau"
        MuSelObj.selName += "noTau"
        ElSelObj.yieldTitle += " + Tau Veto"
        MuSelObj.yieldTitle += " + Tau Veto"

        ElSelObj.refine(cut = [op.rng_len(self.tauCleanSel) == 0])
        MuSelObj.refine(cut = [op.rng_len(self.tauCleanSel) == 0])


    #---- Tight selection ----#
    ElSelObj.selName += "TightSelected"
    MuSelObj.selName += "TightSelected"
    ElSelObj.yieldTitle += " + Tight selection"
    MuSelObj.yieldTitle += " + Tight selection"
    ...
    if use_dd:
        ...
        ElSelObj.create(cut      = [lambda_tight_ele(self.electronsFakeSel[0]),
                           ...
                           enable   = enable)
        MuSelObj.create(cut      = [lambda_tight_mu(self.muonsFakeSel[0]),
        ...
        ElSelObj.refine(cut    = [lambda_tight_ele(self.electronsFakeSel[0]),
                                     op.rng_len(self.electronsTightSel) == 1,
                                     op.rng_len(self.muonsTightSel) == 0,
                                     self.electronsTightSel[0].idx == self.electronsFakeSel[0].idx],
                           weight = ElTightSF(self.electronsTightSel[0]))
        MuSelObj.refine(cut    = [lambda_tight_mu(self.muonsFakeSel[0]),
                                     op.rng_len(self.muonsTightSel) == 1,
                                     op.rng_len(self.electronsTightSel) == 0,
                                     self.muonsTightSel[0].idx == self.muonsFakeSel[0].idx],
                           weight = MuTightSF(self.muonsTightSel[0]))
